#Figure 1d and 1e - XGBoost and SHAP mediator importance for severe COVID-19
...
#Create matrix of relevant biomarker data for input to XGBoost - exclude GM-CSF, G-CSF, IL-3 given large prop. values <LLD
...
#Create outcome for variable to be classified/predicted, in this case severe COVID
predictsevere <- covid[,"whoseverity_imp"] == "4"

## Performing a grid search for Hyperparameter Tuning
### Tuned Parameters - "nrounds", "eta", and "max_depth"
grid_label <- factor(predictsevere, labels = c("nonsevere", "severe"))
...
#Run XGBoost to predict severe illness
library(xgboost)
...
bst <- xgboost(data = covidxgbmatrix, label = predictsevere, nround = 200, params = params)                     
